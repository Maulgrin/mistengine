...
function onInit()
	...
	if (sName or "") ~= "" then
		...
		_ctrlButton = window.createControl("combobox_button_legacy", sButton);
	...
	-- Add a new field to store the selected value
    if node then
		local valueFieldName = getName() .. "value";
		if valuefield then
			-- Use a valuefield override
			valueFieldName = valuefield[1];
		end
		valueFieldNode = DB.createChild(DB.getParent(node), valueFieldName, "string");
    end;
end

function updateValueField(newValue)
	if valueFieldNode then
        DB.setValue(valueFieldNode, "", "string", getValueFor(newValue));
	end
end
...
function getLockMode()
	return self.isComboBoxReadOnly();
end
...
function showList()
	...
	if not _ctrlList and (sName or "") ~= "" then
		...
		if unsorted then
			_ctrlList = window.createControl("combobox_list_legacy", sList);
		else
			_ctrlList = window.createControl("combobox_list_sorted_legacy", sList);
		end
		...
		_ctrlScroll = window.createControl("combobox_scrollbar_legacy", sListScroll);
...
function setListIndex(nIndex)
	if (nIndex > 0) or (nIndex <= #_tOrderedItems) then
		self.setListValue(_tItems[_tOrderedItems[nIndex]].text);
	else
		self.setListValue("");
	end
end
...
function add(sValue, sText, bAllowDelete)
	...
	if type(sValue) == "string" then
		...
		if sText == getValue() then
			updateValueField(sText);
		end
...
function remove(sValue)
	...
	if type(sValue) == "string" then
		...
		if nIndexToRemove > 0 then
			...
			if (getValue() == sValue) or (getSelectedValue() == sValue) then
				self.setListValue("");
			end
		end
	end
...
function optionClicked(wNewSelection)
	if wNewSelection then
		self.setListIndex(wNewSelection.Order.getValue());
	else
		self.setListIndex(0);
	end
...
function getValueFor(newValue)
	for k,v in pairs(_tItems) do
		if v.text == newValue then
			return k;
		end
	end
	return nil;
